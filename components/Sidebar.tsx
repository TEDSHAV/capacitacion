...
const Sidebar = ({ departamentos }: SidebarProps) => {
  ...
  const isActiveDepartment = useCallback(
  ...
  const isActiveSubmodule = useCallback(
    (path: string) => {
  ...
  return (
    <div
      ...
      } shadow-2xl fixed sm:static left-0 top-0 z-40 sm:z-auto`}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      {/* Top Section - Same height as navbar (h-20) */}
      <div
        className={`border-b border-gray-700/50 flex items-center justify-center h-20 backdrop-blur-sm bg-gray-900/80 ${
      ...
      >
        ...
        {user && pathname !== "/dashboard" ? (
          ...
          >
            <ChevronLeft
              className={`text-white transition-transform duration-200 group-hover:scale-110 ${isCollapsed ? "w-4 h-4" : "w-5 h-5"}`}
            />
          </button>
        ) : (
      ...
      >
        ...
        <div className="space-y-2">
          {departamentos.map((department) => {
            ...
            const departmentSubmodules =
              submodules[department.nombre as keyof typeof submodules] || [];
                      ...
                      {departmentSubmodules.map((submodule, index) => (
                        ...
                        >
                          <div className="flex items-center space-x-2 min-w-0">
                            ...
                            ></div>
                            <span className="transition-colors duration-300 group-hover:text-white truncate">
                              {submodule.name}
                            </span>
                          </div>
                        </button>
      ...
      <div
        className={`p-4 border-t border-gray-700/50 backdrop-blur-sm bg-gray-900/80 ${
      ...
      >
        {isCollapsed ? (
          ...
          >
            ...
            <span className="text-sm font-medium transition-colors duration-200 group-hover:text-gray-200">
              Contraer
            </span>
          </button>
        )}
