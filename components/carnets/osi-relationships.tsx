...
export function OSIRelationships({ osiId, className = "" }: OSIRelationshipsProps) {
  const [relationships, setRelationships] = useState<CarnetRelationships | null>(null);
  ...
  const getCertificationStatus = (certificate: CertificateManagement) => {
    if (!certificate.fecha_vencimiento) return { status: 'valid', text: 'Vigente', color: 'green' };
    
    const today = new Date();
    const expirationDate = new Date(certificate.fecha_vencimiento);
    const daysUntilExpiration = Math.ceil((expirationDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
    
    if (daysUntilExpiration < 0) return { status: 'expired', text: 'Vencido', color: 'red' };
    if (daysUntilExpiration <= 30) return { status: 'expiring', text: 'Por vencer', color: 'yellow' };
    return { status: 'valid', text: 'Vigente', color: 'green' };
  };

  const getCarnetStatus = (carnet: Carnet) => {
    if (!carnet.fecha_vencimiento) return { status: 'valid', text: 'Vigente', color: 'green' };
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Resumen de OSI</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <div className="text-2xl font-bold text-blue-600">{relationships.osi.nro_osi}</div>
            <div className="text-sm text-blue-800">Número OSI</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{relationships.certificates.length}</div>
            <div className="text-sm text-green-800">Certificados Emitidos</div>
          </div>
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            <div className="text-2xl font-bold text-purple-600">{relationships.carnets.length}</div>
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">Certificados ({relationships.certificates.length})</h3>
            <svg 
        ...
        {expandedSections.certificates && (
          <div className="p-6">
            {relationships.certificates.length === 0 ? (
              <p className="text-gray-500 text-center py-4">No hay certificados para esta OSI</p>
            ) : (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {relationships.certificates.map((certificate) => {
                      ...
                      const hasCarnet = relationships.carnets.some(c => c.id_certificado === certificate.id);
                      ...
                      return (
                        <tr key={certificate.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {certificate.participantes_certificados?.nombre || 'N/A'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {certificate.fecha_vencimiento 
                              ? new Date(certificate.fecha_vencimiento).toLocaleDateString()
                              : 'No aplica'
                            }
                          </td>
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">Carnets ({relationships.carnets.length})</h3>
            <svg 
        ...
        {expandedSections.carnets && (
          <div className="p-6">
            {relationships.carnets.length === 0 ? (
              <p className="text-gray-500 text-center py-4">No hay carnets para esta OSI</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {relationships.carnets.map((carnet) => {
                  ...
                  return (
                    <div key={carnet.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      ...
                      <div className="space-y-2 text-sm">
                        <div>
                          <span className="font-medium text-gray-700">Participante:</span>
                          <span className="ml-2 text-gray-900">{carnet.nombre_participante}</span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Cédula:</span>
                          <span className="ml-2 text-gray-900">{carnet.cedula_participante}</span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Curso:</span>
                          <span className="ml-2 text-gray-900">{carnet.titulo_curso}</span>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">Emisión:</span>
                          <span className="ml-2 text-gray-900">{new Date(carnet.fecha_emision).toLocaleDateString()}</span>
                        </div>
                        {carnet.fecha_vencimiento && (
                          <div>
                            <span className="font-medium text-gray-700">Vencimiento:</span>
                            <span className="ml-2 text-gray-900">{new Date(carnet.fecha_vencimiento).toLocaleDateString()}</span>
                          </div>
                        )}
                      </div>
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">Participantes Únicos ({relationships.participants.length})</h3>
            <svg 
              ...
              {relationships.participants.map((participant) => (
                <div key={participant.id} className="flex items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">{participant.id_number}</div>
                  </div>
                </div>
              ))}
            </div>
