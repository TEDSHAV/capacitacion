...
export default function Breadcrumb() {
  ...
  return (
    <nav
      ...
      className="bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-2.5"
    >
          ...
          <li key={crumb.href} className="flex items-center gap-1">
            ...
            {crumb.isLast ? (
              <span className="font-medium text-gray-800 truncate max-w-[200px]">
                {idx === 0 && <Home className="w-3.5 h-3.5 inline mr-1 mb-0.5" />}
                {crumb.label}
              </span>
            ) : (
              <Link
                href={crumb.href}
                className="text-gray-500 hover:text-blue-600 transition-colors truncate max-w-[200px] flex items-center gap-1"
              >
