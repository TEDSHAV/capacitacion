...
const getPlantillaCursos = cache(
  async (page: number = 1, limit: number = 10, search: string = "") => {
    ...
    try {
      ...
      if (search) {
        query = query.or(
          `descripcion.ilike.%${search}%,contenido.ilike.%${search}%`,
        );
      }
...
export async function getPlantillaCursosAction(
  page: number = 1,
  limit: number = 10,
  search: string = "",
) {
...
export async function updatePlantillaCursoAction(
  id: number,
  plantillaData: any,
) {
