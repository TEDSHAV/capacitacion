...
export default function FacilitadorHoursStats({
  ...
  const [facilitadorStats, setFacilitadorStats] = useState<
    FacilitadorHoursStat[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [expandedFacilitador, setExpandedFacilitador] = useState<number | null>(
    null,
  );
  ...
  const fetchData = async () => {
    try {
      ...
      const result = await getFacilitatorHoursStatsAction(
        selectedState,
        selectedCourse,
      );
  ...
  const totalHours = facilitadorStats.reduce(
    (sum, stat) => sum + stat.totalCombinedHours,
    0,
  );
  const totalCourses = facilitadorStats.reduce(
    (sum, stat) => sum + stat.totalCertificates,
    0,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-2">
            Promedio Horas/Facilitador
          </h3>
          <p className="text-3xl font-bold text-purple-600">
            {facilitadorStats.length > 0
              ? (totalHours / facilitadorStats.length).toFixed(1)
              : "0"}
          </p>
        </div>
      </div>
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      ...
                      {facilitador.totalCertificates} cursos
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500">
                    ...
                    {facilitador.osiHours.toFixed(1)} OSI
                  </p>
                </div>
              </div>
            ))}
