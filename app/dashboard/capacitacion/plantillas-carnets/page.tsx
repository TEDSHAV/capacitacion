  ...
  const loadPlantillas = async () => {
    try {
      ...
      if (error) {
        ...
        throw new Error(error.message || 'Error al cargar las plantillas de carnet')
      }

      setPlantillas(data || [])
    } catch (error) {
      console.error('Error loading plantillas:', error)
      alert(`Error al cargar las plantillas de carnet: ${error instanceof Error ? error.message : 'Error desconocido'}`)
    } finally {
      setLoading(false)
    }
  }
    ...
    try {
      ...
      const { data: newTemplate, error: insertError } = await supabase
        ...
        .insert({
          ...
          tipo: 'carnet',
          is_active: true, // New template is automatically active
      ...
      alert('¡Plantilla de carnet creada y activada exitosamente!')
    } catch (error) {
      console.error('Error creating plantilla:', error)
      alert(`Error al crear plantilla: ${error instanceof Error ? error.message : 'Error desconocido'}`)
    } finally {
    ...
    return (
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 bg-white">
        <div className="flex items-center justify-center h-64">
          ...
          <span className="ml-2 text-gray-600">Cargando plantillas de carnet...</span>
        </div>
      </div>
    )
  }
    ...
    <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 bg-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Formato recomendado: PNG, tamaño de tarjeta de crédito (85.6mm × 53.98mm)
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {plantillas.map((plantilla) => (
          <div key={plantilla.id} className={`border rounded-lg p-4 hover:shadow-md transition-shadow ${plantilla.is_active ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
            {plantilla.is_active && (
              <div className="mb-2">
            ...
            <div className="aspect-[1.586/1] bg-gray-100 rounded-lg mb-4 overflow-hidden">
              {plantilla.url_imagen && plantilla.url_imagen !== '/templates/carnet.png' ? (
                <img
                  ...
                  onError={(e) => {
                    ...
                    (e.target as HTMLImageElement).src = '/templates/carnet.png';
                  }}
                />
              ) : (
                <img
            ...
            <h3 className="font-semibold text-gray-900 mb-2">{plantilla.nombre}</h3>
            <p className="text-sm text-gray-500 mb-3">
              Archivo: {plantilla.archivo}
            </p>
            <p className="text-xs text-gray-400 mb-4">
              Creada: {new Date(plantilla.created_at).toLocaleDateString('es-VE')}
            </p>
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex space-x-2">
                <button
                  onClick={() => window.open(plantilla.url_imagen || '/templates/carnet.png', '_blank')}
      ...
      {plantillas.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">
            Crea tu primera plantilla de carnet para empezar a personalizar los diseños
          </p>
          <button
