...
const getFacilitatorHoursStats = cache(
  async (stateId?: string, courseId?: string) => {
    ...
    try {
      ...
      if (stateId) {
        query = query.eq("facilitadores.id_estado_geografico", stateId);
      }
      ...
      const facilitatorCourseMap = new Map<
        number,
        Map<number, { courseName: string; hours: number }>
      >();
      const facilitatorInfoMap = new Map<
        number,
        {
      ...
      const result = Array.from(facilitatorHoursMap.entries()).map(
        ([facilitatorId, totalHours]) => {
          const facilitatorInfo = facilitatorInfoMap.get(facilitatorId);
          const courseEntries = Array.from(
            facilitatorCourseMap.get(facilitatorId)?.entries() || [],
          ) as [number, { courseName: string; hours: number }][];
          ...
          return {
            ...
            estado_nombre: getStateName(
              facilitatorInfo?.id_estado_geografico || null,
            ),
...
const getFacilitatorStateStats = cache(
  async (stateId?: string, courseId?: string) => {
    const supabase = await createClient();
    ...
    try {
      ...
      facilitadoresData?.forEach((facilitador) => {
        const geoStateId = facilitador.id_estado_geografico || 0;
      ...
      const statesWithCounts = (allStates || []).map((state) => ({
        ...
        count: geoStateStats.get(state.id) || 0,
      }));
...
const getCourseStats = cache(async (stateId?: string, courseId?: string) => {
  ...
  try {
    ...
    certificates.forEach((cert: any) => {
      ...
      if (!course.facilitadores.has(cert.id_facilitador)) {
        course.facilitadores.set(cert.id_facilitador, {
          ...
          totalCertificates: 0,
          estado_nombre: getStateName(
            facilitatorData.id_estado_geografico || null,
          ),
          is_active: facilitatorData.is_active ?? true,
          certificates: [],
        });
    ...
    // Transform to expected format
    const courseStats = Array.from(courseMap.entries()).map(
      ([courseId, course]) => {
        const facilitadores = Array.from(course.facilitadores.values());
        const totalHours = facilitadores.reduce(
          (sum, f) => sum + f.totalHours,
          0,
        );
        const totalCertificates = facilitadores.reduce(
          (sum, f) => sum + f.totalCertificates,
          0,
        );
...
export async function getFacilitatorHoursStatsAction(
  stateId?: string,
  courseId?: string,
) {
...
export async function getFacilitatorStateStatsAction(
  stateId?: string,
  courseId?: string,
) {
...
export async function getCourseStatsAction(
  stateId?: string,
  courseId?: string,
) {
