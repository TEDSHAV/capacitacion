  ...
  try {
    ...
    const certificateData: CertificateGeneration = {
      ...
      osi_data: {
        ...snapshotData.osi,
        direccion_ejecucion:
          (snapshotData.osi as any)?.direccion_ejecucion ||
          snapshotData.osi?.direccion_fiscal ||
          "",
      },
      ...
      sha_signature_id:
        snapshotData.firmas?.sha_signature_id?.toString() || null,
    ...
    const [shaSignatureData, facilitatorRaw, templateData] = await Promise.all([
      ...
      !(certificateData as any).sha_signature_data
        ? getSignatureDataServer(certificateData.sha_signature_id).catch(
            (e) => {
              console.warn(
                "Failed to fetch SHA signature data for PDF generation:",
                e,
              );
              return null;
            },
      ...
      certificateData.facilitator_id && !certificateData.facilitator_data
        ? getFacilitatorDataServer(certificateData.facilitator_id).catch(
            (e) => {
              console.warn(
                "Failed to fetch facilitator data for PDF generation:",
                e,
              );
              return null;
            },
      ...
      certificateData.id_plantilla_certificado
        ? getCertificateTemplateServer(
            ...
            console.warn(
              "Failed to fetch certificate template, using default:",
              e,
            );
            return null;
          })
    ...
    if (
      ...
      !certificateData.participants[0].name
    ) {
      return NextResponse.json(
    ...
    try {
      ...
      return new NextResponse(pdfBlob, {
        headers: {
          ...
          "Cache-Control": "no-store, max-age=0", // Disable cache to show latest edits
        },
      });
    } catch (pdfError) {
