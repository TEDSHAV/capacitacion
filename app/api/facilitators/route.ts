...
export async function POST(request: NextRequest) {
  try {
    ...
    const id_estado_base = formData.get("id_estado_base")
      ? parseInt(formData.get("id_estado_base") as string)
      : null;
    const id_ciudad_base = formData.get("id_ciudad_base")
      ? parseInt(formData.get("id_ciudad_base") as string)
      : null;
    const id_estado_geografico = formData.get("id_estado_geografico")
      ? parseInt(formData.get("id_estado_geografico") as string)
      : null;
    const id_estatus = formData.get("id_estatus")
      ? parseInt(formData.get("id_estatus") as string)
      : 1;
    const temas_cursos = JSON.parse(
      (formData.get("temas_cursos") as string) || "[]",
    );
    ...
    const calificacion = formData.get("calificacion")
      ...
      : null;
    const firma_id = formData.get("firma_id")
      ? parseInt(formData.get("firma_id") as string)
      : null;
    ...
    if (!nombre_apellido || !cedula || !email || !telefono) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: nombre_apellido, cedula, email, telefono",
        },
        { status: 400 },
      );
    }
    ...
    console.log(
      "Creating facilitator with signature:",
      signatureFile ? signatureFile.name : "No signature file",
    );

    // Handle signature file upload if provided
    if (signatureFile) {
      console.log(
        "Processing signature file:",
        signatureFile.name,
        "Size:",
        signatureFile.size,
        "Type:",
      ...
      .insert([
        {
          ...
          ano_ingreso: fecha_ingreso
            ? new Date(fecha_ingreso).getFullYear()
            : null,
          nombre_apellido,
          cedula,
          rif: rif || null,
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "Successfully fetched facilitators:",
      data?.length || 0,
      "records",
    );
    return NextResponse.json(data);
