  ...
  try {
    ...
    if (!certificates || !osiData || !firmanteData || !documentType) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: certificates, osiData, firmanteData, documentType",
        },
        { status: 400 },
      );
    }
    ...
    console.log(
      "🔧 Using TemplateBasedPdfGenerator for document type:",
      documentType,
    );
    ...
    switch (documentType) {
      ...
      default:
        return NextResponse.json(
          {
            error:
              "Invalid document type. Must be: certificacion_competencias, nota_entrega, or validacion_datos",
          },
          { status: 400 },
        );
    }
    ...
    headers.set(
      "Content-Disposition",
      `attachment; filename="${documentType}_${Date.now()}.pdf"`,
    );
