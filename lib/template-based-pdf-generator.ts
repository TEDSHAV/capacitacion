...
export class TemplateBasedPdfGenerator {
  ...
  private drawBorderedTable(
    ...
    for (const row of allRows) {
      ...
      row.forEach((cell, ci) => {
        ...
        pdf.text(String(cell ?? ""), x + 2, y + rowH / 2 + 1.5, {
  ...
  async generateCertificacionCompetencias(data: TemplateData): Promise<Buffer> {
    try {
      ...
      const certRows: string[][] = data.participantes.map((p, i) => {
        const idPrefix = p.nacionalidad === "extranjero" ? "e-" : "V-";
        const formattedId = `cedula: ${idPrefix}${p.cedula}`;
        return [
          String(p.index ?? i + 1),
          p.nombre_apellido || "",
          formattedId,
  ...
  async generateNotaEntrega(data: TemplateData): Promise<Buffer> {
    try {
      ...
      y += 6;
      pdf.text(data.nombre_recibido || "[NOMBRE Y APELLIDO]", cx, y, {
        align: "center",
      });
      y += 6;
